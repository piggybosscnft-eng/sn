{if !$standalone}<li>{/if}
	...
	<div class="bg-white position-relative p-3 post 
				...
				{if ($post['still_scheduled']) OR ($post['is_pending']) OR ($post['in_group'] && !$post['group_approved']) OR ($post['in_event'] && !$post['event_approved'])}pending{/if}" data-id="{$post['post_id']}" {if $post['is_schedule']}data-schedule="true" {/if}>
				...
				<div class="d-flex mt-2 pt-1">
					...
					<div class="flex-1">
						...
						<div class="post-stats d-flex align-items-center">
							<!-- reactions stats -->
							{include file='__feeds_post.reactions_stats.tpl'}
							<!-- reactions stats -->
